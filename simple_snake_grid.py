...
class SimpleSnakeGrid:
    ...
    def init_virtual_snake_and_grid(self):
        self.snake_vir_grid,snake_vir_head,snake_vir_body,_ = self.get_updated_grid(self.initial_observation)
    ...
    def update_observation(self, observation):
        ...
        self.snake_grid,self.snake_head,_,self.snake_food = self.get_updated_grid(self.observation)
    ...
    def create_grid_from_v_snake(self, virtual_snake_dq):
        rows, cols = (self.initial_observation.shape[0], self.initial_observation.shape[1])
        arr = [[0 for i in range(cols)] for j in range(rows)]
    ...
    def bfs_actions(self):
        ...
        follow_tail = self.bfs(virtual_snake_curr[-1], look_ahead_grid, virtual_snake_curr[0])
